...
pub fn clear_launch_password() -> Result<(), String> { let e = keyring::Entry::new(SERVICE, PW_HASH).map_err(|e| e.to_string())?; match e.delete_password() { Ok(_) | Err(keyring::Error::NoEntry) => Ok(()), Err(e) => Err(e.to_string()) } }
